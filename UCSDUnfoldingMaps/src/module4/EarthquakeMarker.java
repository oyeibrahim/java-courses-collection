...
public abstract class EarthquakeMarker extends SimplePointMarker
{
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		if ("Past Day".equals(age)) {
			pg.strokeWeight(2);
			//line will extend a little beyond the circle
			int buffer = 2;
	...
	private void colorDetermine(PGraphics pg) {
		...
		float depth = getDepth(); 
		 //shallow... depth less than intermidiate is shallow
		if (depth < THRESHOLD_INTERMEDIATE) { 
			pg.fill(255, 255, 0); // yellow 
		} 
		//intermidiate... depth less than deep is intermidiate
		else if (depth < THRESHOLD_DEEP) { 
			pg.fill(0, 0, 255); // blue 
		} 
		//deep... whatever is left will be in deep category
		else { 
			pg.fill(255, 0, 0); // red 
		} 
	}
